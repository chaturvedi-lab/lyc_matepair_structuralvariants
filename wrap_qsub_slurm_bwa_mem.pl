...
my $dir = '/uufs/chpc.utah.edu/common/home/gompert-group1/data/lycaeides/whole_genomes/sequences';
my @jobarray;
my $aln;
#my $samse;
my $ind;
...
my $resdir = '/uufs/chpc.utah.edu/common/home/gompert-group1/data/lycaeides/lyc_sv/Alignments/'; 
my $cd = "cd $dir\n";
...
for my $file (@ARGV){
	$file =~ m/([a-z]+\_3kb\_[12]\.fastq\.gz)/ or die "failed sub $file\n";
	#print "$file";
    	$id = $1;
	#print "$id";
    	$ind = "$1";
    ...
    #print "$in $in2 $ind $run $pnum\n";
    	$aln ="bwa mem -t 12 -k 15 -r 1.3 -T 30 -R \'\@RG\\tID:$ind\\tPL:ILLUMINA\\tLB:$ind\\tSM:$ind"."\' $genome $in1 $in2 > $resdir"."$ind".".sam\n";
